...
import { useMarquee } from "@/lib/hooks/use-gsap";
...
const Navbar: React.FC<NavbarProps> = ({ initialNotices = [] }) => {
    ...
    initialNotices.length > 0
      ...
      : [
          ...
          },
          {
            id: "3",
            title:
              "বিটিসি এর মাধ্যমে আপনার প্রজেক্টের সঠিক পরিকল্পনা এবং বাস্তবায়ন নিশ্চিত করুন।",
            link: "/about",
          },
        ],
  );

  // Handle scroll to toggle fixed class and active section
  useEffect(() => {
    const handleScroll = () => {
      // Fixed Navbar logic
  ...
  // Intersection Observer for more precise triggering (beshi percent ashle active)
  useEffect(() => {
    if (pathname !== "/") return;
  ...
  const { marqueeRef, handleMouseEnter, handleMouseLeave } = useMarquee(
    50,
    notices,
  );
  ...
  const isLinkActive = (path: string, sectionId?: string) => {
    ...
    return activeSection === "home" && path === "/";
  };

  return (
    <div>
      {/* Top Notice Board */}
      <div className="top_notice_board py-[6px] bg-[#ff253a]">
        <div className="text_marquee flex items-center overflow-hidden">
          {notices.length === 0 ? (
            <span className="white-text-important text-[16px] font-[400] !font-bangla inline-block mx-auto">
            ...
            >
              {notices.map((notice) => (
                <Link
                  key={notice.id}
                  href={notice.link}
                  className="js_text text-white white-text-important text-[16px] font-[400] !font-bangla inline-block mr-[30px] pr-[30px] relative whitespace-nowrap hover:text-white/80 transition-colors"
                >
                  {notice.title}
                  <span className="absolute top-[3px] right-0 w-[1px] h-[75%] bg-white opacity-30"></span>
                </Link>
              ))}
            </div>
          )}
      ...
      {/* Top Bar */}
      <div className="topbar hidden lg:block bg-white">
          ...
          <div className="flex justify-between items-center py-2.5">
            <div className="flex justify-between items-center">
              <div className="flex items-center gap-1">
                ...
                <a
                  ...
                  className="text-text-1 text-base font-normal font-secondary ml-0.5 hover:text-primary transition duration-300"
                >
            ...
            <div className="flex items-center gap-2">
              <a
                ...
                className="hover:text-primary transition duration-400 mt-1"
              >
                <i className="fa-brands fa-facebook-f text-text-1 text-xl hover:text-primary transition duration-400"></i>
              </a>
              ...
              <div className="flex items-center gap-1">
                ...
                <a
                  ...
                  className="nav-link-item text-text-1 text-base font-normal font-secondary ml-0.5 transition duration-300"
                >
                  info@egpbtc.com
                </a>
              </div>
            ...
            <div className="flex justify-between h-[80px] items-center px-3">
              ...
              <div className="hidden lg:flex space-x-6 items-center">
                ...
                <Link
                  href="/#services"
                  className={`nav-link-item transition duration-400 font-medium ${
                    isLinkActive("/#services", "services") ? "active" : ""
                  }`}
                >
                  SERVICES
                </Link>
                <Link
                  href="/#projects"
                  className={`nav-link-item transition duration-400 font-medium ${
                    isLinkActive("/#projects", "projects") ? "active" : ""
                  }`}
                >
                  PROJECTS
                </Link>
              ...
              <div className="hidden lg:block">
                <Link
                  href="/contact"
                  className={`contact_btn cursor-pointer relative inline-flex items-center justify-center px-8 py-2.5 overflow-hidden tracking-tighter text-white bg-primary rounded-tl-lg rounded-tr-lg rounded-bl-0 rounded-br-lg group transition-all duration-300 ${
                    pathname === "/contact"
                ...
                >
                  ...
                  <span className="relative text-base font-semibold text-white group-hover:text-white transition-colors duration-300 uppercase">
                    Contact Us
                  </span>
                </Link>
              </div>
      ...
      {/* Mobile Menu */}
      <div
        className={`fixed overflow-auto top-0 left-0 h-full w-64 bg-white shadow-lg transform ${
      ...
      >
        <div className="flex justify-between items-center px-4 py-3 border-b">
          ...
          <button onClick={toggleMobileMenu} className="text-text-1">
            <svg
              className="w-6 h-6"
              fill="none"
              stroke="currentColor"
        ...
        <div className="px-4 py-2 space-y-1">
          ...
          <Link
            href="/#services"
            className={`block px-3 py-2 rounded-lg transition-all duration-300 ${
              isLinkActive("/#services", "services")
                ? "text-primary bg-shade-1"
                : "text-text-1 hover:text-primary hover:bg-shade-1"
            }`}
            onClick={toggleMobileMenu}
        ...
        <div className="px-4 py-2">
          <div className="border-t border-t-ac-1"></div>
          <div className="flex items-center gap-6 my-5">
            <a
              href="https://www.facebook.com/egpbtc"
              target="_blank"
              rel="noopener noreferrer"
              className="hover:text-primary transition duration-400"
            >
              <i className="fa-brands fa-facebook-f text-text-1 text-xl hover:text-primary transition duration-400"></i>
            </a>
            <a href="#" className="hover:text-primary transition duration-400">
              <i className="fa-brands fa-instagram text-text-1 text-xl hover:text-primary transition duration-400"></i>
            </a>
            <a href="#" className="hover:text-primary transition duration-400">
              <i className="fa-brands fa-x-twitter text-text-1 text-xl hover:text-primary transition duration-400"></i>
            </a>
            <a href="#" className="hover:text-primary transition duration-400">
              <i className="fa-brands fa-linkedin-in text-text-1 text-xl hover:text-primary transition duration-400"></i>
            </a>
          </div>
          <div className="block lg:hidden">
            <Link
              href="/contact"
              className="w-full cursor-pointer relative inline-flex items-center justify-center px-8 py-2.5 overflow-hidden tracking-tighter text-white bg-primary rounded-tl-lg rounded-tr-lg rounded-bl-0 rounded-br-lg group"
              onClick={toggleMobileMenu}
            >
              <span className="absolute bottom-0 left-0 right-0 h-0 transition-all duration-500 ease-out bg-text-1 group-hover:h-full"></span>
              <span className="relative text-base font-semibold uppercase">
                Contact Us
              </span>
            </Link>
          </div>
      ...
      {isMobileMenuOpen && (
        <div
          className="fixed top-0 left-0 w-full h-full bg-black/30 z-[51]"
          onClick={toggleMobileMenu}
        ></div>
      )}
    </div>
