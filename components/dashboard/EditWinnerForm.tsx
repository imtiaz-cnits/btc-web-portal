...
export default function EditWinnerForm({ winner }: { winner: any }) {
  ...
  // Custom states
  const [status, setStatus] = useState(winner.status || "active");
  const [openStatusDropdown, setOpenStatusDropdown] = useState(false);
  const [publishDate, setPublishDate] = useState(
    winner.publishDate ? new Date(winner.publishDate).toISOString().split('T')[0] : ""
  );
    ...
    function initFlatpickr() {
      if ((window as any).flatpickr) {
        (window as any).flatpickr(".flatpickr-date", {
          dateFormat: "Y-m-d",
          allowInput: true,
          defaultDate: winner.publishDate ? new Date(winner.publishDate).toISOString().split('T')[0] : "",
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="md:col-span-2 space-y-2">
          <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Replace Result Document (Optional)</label>
          <input 
            type="file" 
            name="file" 
            accept=".pdf,image/*"
          ...
          />
          {winner.filePath && (
            <p className="text-[11px] text-slate-400 italic font-semibold mt-1">📎 Current File: <a href={winner.filePath} target="_blank" rel="noreferrer" className="text-[var(--primary-color)] underline hover:text-green-700 font-bold">{winner.filePath.split('/').pop()}</a></p>
          )}
        </div>
      </div>
