...
const Dashboard = () => {
  ...
  const [errorWinners, setErrorWinners] = useState(null);
    ...
    const fetchNotices = async () => {
      try {
        ...
        if (response.data.success) {
          ...
          throw new Error(response.data.message || 'Failed to load notices');
        }
      } catch (error) {
        console.error('Error fetching notices:', error);
        setErrorNotices('Failed to load notices');
      } finally {
  ...
  useEffect(() => {
    const fetchWinners = async () => {
      try {
        ...
        if (response.data.success) {
          const winnersData = response.data.notices || response.data.data || [];
        ...
        setErrorWinners('Failed to load winner list');
      } finally {
  ...
  return (
    <div className="">
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
        {/* Card 1 */}
        {/*<div className="db-card flex flex-col md:flex-row text-center md:text-left items-center gap-3 bg-[var(--bg)] dark:bg-[var(--dark-bg2)] rounded-[16px] shadow-md overflow-hidden hover:shadow-lg transition-shadow p-[10px]">*/}
        {/*  <div*/}
      ...
      <div className="mt-8" style={{ marginTop: '20px' }}>
        <h2 className="text-xl font-bold text-[var(--text-1)] dark:text-[var(--text-4)] mb-4">EGP Notices</h2>
        <div className="bg-[var(--bg)] dark:bg-[var(--dark-bg2)] rounded-[16px] shadow-md overflow-hidden p-4">
          <div className="overflow-x-auto">
            <table className="w-full text-left text-[var(--text-2)]">
              <thead>
                <tr className="border-b border-[var(--text-2)]/20">
                ...
                {loadingNotices ? (
                  ...
                  notices.slice(0, 3).map((notice, index) => (
                    <tr
                      ...
                      className="hover:bg-[var(--secondary-color)]/10 transition-colors"
                    >
                      <td className="py-3 px-4">{index + 1}</td>
                      <td className="py-3 px-4">{notice.title}</td>
                      <td className="py-3 px-4">{formatDate(notice.publishDate)}</td>
                      <td className="py-3 px-4">{formatDate(notice.lastDate)}</td>
                      ...
                      <td className="py-3 px-4">
                        {notice.filePath ? (
                          <a
                            href={`https://egpbtc.com${notice.filePath}`}
                            target="_blank"
      ...
      {/* Winner List Table */}
      <div className="mt-8" style={{ marginTop: '20px' }}>
        <h2 className="text-xl font-bold text-[var(--text-1)] dark:text-[var(--text-4)] mb-4">Winner List</h2>
        <div className="bg-[var(--bg)] dark:bg-[var(--dark-bg2)] rounded-[16px] shadow-md overflow-hidden p-4">
          <div className="overflow-x-auto">
            <table className="w-full text-left text-[var(--text-2)]">
              <thead>
                <tr className="border-b border-[var(--text-2)]/20">
                  ...
                  <th className="py-3 px-4 font-semibold">Title</th>
                  <th className="py-3 px-4 font-semibold">Publish Date</th>
                  <th className="py-3 px-4 font-semibold">Lottery Date</th>
                  <th className="py-3 px-4 font-semibold">Status</th>
                  <th className="py-3 px-4 font-semibold">File</th>
                </tr>
              </thead>
              <tbody>
                {loadingWinners ? (
                  ...
                  winners.slice(0, 3).map((winner, index) => (
                    <tr
                      key={winner._id || winner.id || index}
                      className="hover:bg-[var(--secondary-color)]/10 transition-colors !text-black dark:!text-white"
//                      style={{ color: 'black' }}
                    >
                      <td className="py-3 px-4">{index + 1}</td>
                      <td className="py-3 px-4">{winner.title}</td>
                      <td className="py-3 px-4">{formatDate(winner.publishDate)}</td>
                      <td className="py-3 px-4">{formatDate(winner.lastDate)}</td>
                      <td className="py-3 px-4">
                        <span
                          className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                      ...
                      <td className="py-3 px-4">
                        {winner.filePath ? (
                          <a
                            href={`https://egpbtc.com${winner.filePath}`}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="text-blue-600 hover:underline"
                          >
